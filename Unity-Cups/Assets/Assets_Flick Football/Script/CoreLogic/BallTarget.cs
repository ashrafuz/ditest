...
{
	private class DelayedLogger
	{
		public delegate string ContentFunction();
		
		private int count = 0;
		private int delay;	
		private ContentFunction content;
		
		public DelayedLogger(ContentFunction f, int delay) {
			this.content = f;
			this.delay = delay;
		}
		...
		public void Update() {
			...
			if (count >= delay) {
				count = 0;
				MainMenu.NativeLog(this.content());
			}
		}
	}
    ...
    void Start() {
		logger = new DelayedLogger(() => "Ball position: " + _ballTarget.position.ToString(), 60);
